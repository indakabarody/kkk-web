        ...
        <div class=" container-xxl " id="kt_content_container">
            ...
            <div class="card mb-5 mb-xl-10">
                ...
                <div id="kt_account_profile_details" class="collapse show">
                    {{-- begin::Form --}}
                    <form class="form" action="{{ route('admin.income-transactions.update', $transaction->id) }}"
                                    ...
                                    <select name="student_id"
                                        ...
                                        @foreach ($students as $student)
                                            <option value="{{ $student->id }}"
                                                @if ($transaction->student_id == $student->id) selected @endif>{{ $student->nim }} -
                                                {{ $student->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-6">
                                ...
                                <div class="col-lg-8 fv-row">
                                    <select class="form-select form-select-solid @error('category') is-invalid @enderror"
                                        ...
                                        <option value="Iuran" @if ($transaction->category == 'Iuran') selected @endif>Iuran
                                        </option>
                                        <option value="Denda" @if ($transaction->category == 'Denda') selected @endif>Denda
                                        </option>
                                        <option value="Lainnya" @if ($transaction->category == 'Lainnya') selected @endif>Lainnya
                                        </option>
                                    </select>
                                    @error('category')
                                        <div class="invalid-feedback">{{ $message }}</div>
